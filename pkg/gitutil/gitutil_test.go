	...
	g.Describe("gitutil", func() {
		...
		g.Describe("ApplyInsteadOf", func() {
			g.It("works", func() {
				...
				url, err := ApplyInsteadOf("git@github.com/common-repo/common-repo-v1")
				Expect(err).ShouldNot(HaveOccurred())
				Expect(url).To(HaveSuffix("common-repo/common-repo-v1"))
			})
		})
		...
		g.Describe("(external)", func() {
			...
			g.Describe("DefaultRef", func() {
				g.It("returns the default head when it's not master", func() {
					url := "https://github.com/common-repo/common-repo-v1"
